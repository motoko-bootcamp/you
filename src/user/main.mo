...
shared ({ caller = creator }) actor class UserCanister(
    ...
    public func reboot_getInformation() : async (Name, Int, Principal) {
        return (name, born, owner);
    };

    public query func reboot_getAge() : async Int {
        let now : Int = Time.now();
        return Float.toInt(Float.fromInt(now - born) / (365.25 * 1_000_000_000.0 * 24.0 * 60.0 * 60.0));
    };

    public query func reboot_getAgeInDays() : async Int {
        let now : Int = Time.now();
        return (now - born) / nanosecondsPerDay;
    };

    public query func reboot_getAgeInSeconds() : async Int {
        let now : Int = Time.now();
        let nanosecondsPerSecond : Nat = 1_000_000_000;
        return (now - born) / nanosecondsPerSecond;
    };
